...
import { NightInfo, Interval, CoordSeries, Az, normalizeRad, radToDeg } from 'astroffers-core';
...
export default class extends React.PureComponent<{ horizontalCoords: CoordSeries<Az> }> {
  render() {
    return <HighCharts config={getConfig(this.props.horizontalCoords)} style={{ width: 200, height: 200 }} />;
  }
}
...
const getConfig = (horizontalCoords: CoordSeries<Az>) => {
  ...
  return {
    ...
    tooltip: {
      borderWidth: 0,
      formatter: function() {
        return `${window['moment'](this.point.time).format('HH:mm')} - Az: ${Math.round(this.x)}° Alt: ${Math.round(
          90 - this.y
        )}°`;
      }
    },
